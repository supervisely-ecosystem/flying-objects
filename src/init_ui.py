...
def init_classes_stats(api: sly.Api, data: dict, state: dict, project_info, project_meta: sly.ProjectMeta):
    ...
    class_images = {item["objectClass"]["name"]: item["total"] for item in stats["images"]["objectClasses"]}
    class_objects = {item["objectClass"]["name"]: item["total"] for item in stats["objects"]["items"]}
