...
def postprocess(
    ann: sly.Annotation, cur_meta: sly.ProjectMeta, res_meta: sly.ProjectMeta
) -> (sly.ProjectMeta, sly.Annotation):
    task_type = g.STATE.SETTINGS.task_type

    sly.logger.debug(f"Started postprocessing with task type {task_type}.")

    if task_type == "segmentation":
        sly.logger.debug("Detected task as segmentation.")
        new_meta, new_ann = transform_for_segmentation(cur_meta, ann)
    elif task_type == "detection":
        sly.logger.debug("Detected task as detection.")
        new_meta, new_ann = transform_for_detection(cur_meta, ann)
    elif task_type == "instance_segmentation":
        sly.logger.debug("Detected task as instance segmentation.")
...
def transform_for_detection(
    ...
    for label in ann.labels:
        new_class = label.obj_class.clone(
            name=label.obj_class.name + "-bbox", geometry_type=sly.Rectangle
        )
...
def transform_for_segmentation(
    meta: sly.ProjectMeta, ann: sly.Annotation
) -> (sly.ProjectMeta, sly.Annotation):
    new_classes = {}
    class_masks = {}
...
def transform_for_instance_segmentation(
    meta: sly.ProjectMeta, ann: sly.Annotation
) -> (sly.ProjectMeta, sly.Annotation):
    new_classes = {}
    for obj_class in meta.obj_classes:
...
def merge_classes(
    cur_meta: sly.ProjectMeta, res_meta: sly.ProjectMeta, ann: sly.Annotation
):
