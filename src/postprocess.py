...
def postprocess(
    state, ann: sly.Annotation, cur_meta: sly.ProjectMeta, res_meta: sly.ProjectMeta
...
def transform_for_detection(
    ...
    for label in ann.labels:
        new_class = label.obj_class.clone(
            name=f"{label.obj_class.name}-bbox", geometry_type=sly.Rectangle
        )
...
def transform_for_segmentation(
    ...
    for obj_class in meta.obj_classes:
        ...
        new_class = obj_class.clone(name=f"{obj_class.name}-mask")
...
def merge_classes(
    ...
    existing_names = {obj_class.name for obj_class in res_meta.obj_classes}
    mapping = {}
